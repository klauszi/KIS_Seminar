  ...
  \begin{block}<+->{Zeitreihenreparatur}
    \begin{itemize}
        \item Gegeben:
           ...
           \end{itemize}
                \item Nur in der Evaluierung: vollständige, korrekte Messung $x^{\text{truth*}}$
        \item Gesucht:
            \begin{itemize}
...
\begin{frame}{Reparatur durch Anomalienerkennung}
  \begin{block}<+->{Autoregressive Modell $AR(p)$}
    \begin{itemize}
      \item Prädiktion aus den vorangegangen $p$ Werte:\\
...
\end{columns}
\end{frame}
\begin{frame}{Parameter Abschätzung}
  \begin{block}<+->{AR(1)}
      \[
      X_t = \phi X_{t-1} + \epsilon_t
      \]
  \end{block}
  \begin{block}<+->{Kleinste Quadrate Schätzung}
      \begin{align}
...
\begin{frame}{Parameter Abschätzung}
  ...
  \end{block}
\end{frame}
\begin{frame}{Parameter Abschätzung $p=1$}
...
\begin{frame}{Glättungsverfahren}
...
\begin{frame}{Bedingung basiert Verfahren}
  \begin{block}<+->{SCREEN}
    \begin{itemize}
      \item Die Reparatur erfolgt nur über zwei aufeinander folgende Werte 
      \item Bedingung wie schnell ein Wert sich ändern kann
      \item gut für spikes aber nicht für Fehlern über längeren Zeitraum
    \end{itemize}
  \end{block}
\end{frame}
